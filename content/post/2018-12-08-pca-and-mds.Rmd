...
<a name="later"></a> Consider a projection $P$ onto a two dimensional subspace of $\mathbb{R}^3$ and let $Y=XP$. Since $X$ is centered, so is $Y$ and hence the total variance of $Y$ is given by $\frac{1}{n-1}\|Y\|_{HS}^2$, where $\|\cdot\|_{HS}$ denotes the Hilbert-Schmidt norm. 
...
In particular, this shows that the first two columns of $V$ are basis for $XP_0$ 
and the first two columns of $U\Sigma_0$ provide the desired coordinates $\mathbf{Q}_1, \cdots, \mathbf{Q}_{`r n`}$. This can be obtained as in the following using an R function `svd()`:

```{r}
SVD <- svd(X, nu = n)
U <- SVD$u # 100 x 100
D <- rbind(diag(SVD$d), matrix(0, nrow = n-3, ncol = 3)) # 100 x 3
V <- SVD$v
X_recover <- (U %*% D) %*% t(V) # This recovers X 
Q <- U %*% D # The first two columns of Q gives coordinates Q1, Q2, ..., Q100
head(Q[, 1:2])
```

The R function `prcomp()` easily computes the coordinates $\mathbf{Q}_1, \cdots, \mathbf{Q}_{`r n`}$:
