...
fit <- glm(IsVirginica ~ Sepal.Length, 
           data = dIris,
...
The following model is saved as `simple_logistic.stan`:

```{bash, comment = ""}
cat simple_logistic.stan
```
...
irisData <- list(N = nrow(dIris),
                 y = dIris$IsVirginica,
                 x = dIris$Sepal.Length)
fit_logit <- stan(file = 'simple_logistic.stan',
                  data = irisData,
                  seed = 2019,
                  chains = 5,
                  iter = 10000,
